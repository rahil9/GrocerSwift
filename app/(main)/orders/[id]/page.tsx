...
export default function OrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      if (savedOrders) {
        try {
          ...
          if (userOrder) {
            ...
            const hoursSinceOrder = (currentTime - orderDate) / (1000 * 60 * 60)

            // Update order status based on delivery method and time
            const updatedOrder = { ...userOrder }

            if (userOrder.status === "processing") {
              if (userOrder.deliveryMethod === "express" && hoursSinceOrder >= 10 / 60) {
                // 10 minutes for express
                updatedOrder.status = "delivered"
              } else if (userOrder.deliveryMethod === "standard" && hoursSinceOrder >= 24) {
                // 24 hours for standard
                updatedOrder.status = "delivered"
              } else if (hoursSinceOrder >= 1) {
                // After 1 hour, mark as shipped
  ...
  const getEstimatedDeliveryDate = () => {
    ...
    if (order.deliveryMethod === "express") {
      // Add 10 minutes for express delivery
      const deliveryDate = new Date(orderDate.getTime() + 10 * 60 * 1000)
      return deliveryDate.toLocaleString()
    } else {
      // Add 24 hours for standard delivery
